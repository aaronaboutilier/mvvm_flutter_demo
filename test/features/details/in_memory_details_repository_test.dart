...
void main() {
  ...
  final repo = InMemoryDetailsRepository(foundation.PerformanceMonitor(const foundation.DebugLogger()));
  expect((await repo.getItems()), isA<foundation.Success<List<DetailItem>>>());
  ...
  final repo = InMemoryDetailsRepository(foundation.PerformanceMonitor(const foundation.DebugLogger()));
      ...
      // invalid indexes
  expect(await repo.reorder(oldIndex: -1, newIndex: 1), isA<foundation.FailureResult<void>>());
  expect(await repo.reorder(oldIndex: 0, newIndex: 5), isA<foundation.FailureResult<void>>());
      // valid reorder
  final before = (await repo.getItems() as foundation.Success<List<DetailItem>>).value;
      expect(before.map((e) => e.name).toList(), ['A', 'B', 'C']);
      await repo.reorder(oldIndex: 0, newIndex: 2);
  final after = (await repo.getItems() as foundation.Success<List<DetailItem>>).value;
